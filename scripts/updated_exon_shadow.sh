...
#filter & classiffy exon-scfr overlaps & calculate exon shadow
  #create output files
  echo "chr start end frame filler strand chr exon_start exon_end gene transcript exon_strand exon_frame exon_number gene_tx_count exon_tx_count exon_order exon_sharing exon_splicing upstream_len_in_scfr downstream_len_in_scfr" | tr " " "\t" > "$species"_single_exon.tsv
