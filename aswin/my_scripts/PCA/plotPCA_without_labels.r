...
if (length(args) != 2) {
    stop("Usage: Rscript plotPCA.R <input_dir> <output_dir>")
}
...
# -----------------------------
# Read input RSCU table
# -----------------------------
rscu_file <- file.path(input_dir, "rscu.tsv")

if (!file.exists(rscu_file)) {
    stop(paste("ERROR: rscu.tsv not found in", input_dir))
}

# Read RSCU data
rscu <- read.table(rscu_file, header = TRUE, sep = "\t", stringsAsFactors = FALSE)
...
palette_colors <- setNames(brewer.pal(max(n_clades, 8), "Dark2")[1:n_clades], clades)
...
plot_and_save_pca <- function(df, loadings, x, y, outname, top_n = 5) {
  ...
  # PCA plot (NO species labels)
  p <- ggplot(df, aes_string(x = x, y = y, color = "clade")) +
...
for (pair in combinations) {
  ...
  plot_and_save_pca(pca_df, loadings, x, y, outpath, top_n = 5)
}
